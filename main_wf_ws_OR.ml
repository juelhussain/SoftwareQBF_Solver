   ...
   ;;
	...
	let get_var_num clause_list = 
		...
		begin
			...
			else begin Printf.printf "get_var_num: The header: %s" (header);"NA "end
		end
         ...
         let str = read_line () in
           try
							...
							(*(exp_size/(if (exp_size > 100) then exp_size/20 
								...
								let segment_or = 2 in
								Printf.printf "QBF_Solver- Segmentation Disjunction value: %d\n" segment_or;
							let con_bdd_list = Syntax.time5 "Process" Manager.start_process_segmentation_double (clauseList) (h) (t) (segment_val) (segment_or)
							in
