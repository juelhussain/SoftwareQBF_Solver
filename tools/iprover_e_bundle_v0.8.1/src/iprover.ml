...
let create_name_symb_table () =
  ...
  in
  SymbolDB.fold f !Parsed_input_to_db.symbol_db_ref NameSymMap.empty
...
(* when Sat and eq ax are omitted we need to add them and start again *)
  ...
  then
    ...
    try    
      ...
      |Schedule_default           -> schedule_run clauses (default_schedule ())
      |Schedule_verification_epr  -> schedule_run clauses (verification_epr_schedule ())
      |Schedule_none              ->
...
(* Parser_types.FOF_format then parse with a clausifier *)
	  ...
	  let clausifier_parse () = 
	    (input_problem_type:= Some Parser_types.FOF;	   
	     ParseFiles.clausify_parse !current_options.problem_files
	    ) 
	  in
