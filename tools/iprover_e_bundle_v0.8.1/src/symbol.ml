...
let to_string_sproperty = function
  ...
  |Flat             -> "flat"
  |FunPred          -> "funpred"
  |DomainConstant   -> "domain constant"
  |DomainPred       -> "domain predicate"
  |DefPred          -> "defpred"
...
let to_stream_full s symb = 
  ...
  s.stream_add_str (param_to_string to_string_sproperty symb.sproperty);
  s.stream_add_char '\n'
