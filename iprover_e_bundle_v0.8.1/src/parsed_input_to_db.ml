...
(*(* applicable only for E *)
	(if str = "# CNFization successful!" then
	  (e_prover_parsing_success:= true));
	if str = "# SZS status: ResourceOut" then 	
	  failwith "eprover:  # SZS status: ResourceOut" 
	else 
*)
	f tl
